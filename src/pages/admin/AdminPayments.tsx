  ...
  const filteredOrders = useMemo(() => {
    return orders.filter(order => {
      ...
      if (searchQuery.trim()) {
        ...
        if (!matchesId && !matchesEmail && !matchesName && !matchesPlan && !matchesGatewayId && !matchesInvoice && !matchesFailure) {
          return false;
        }
      }
                  ...
                  return (
                    <tr key={order.id} className="hover:bg-muted/20 transition-colors group">
                      ...
                      <td className="py-3.5 px-4 align-top">
                        ...
                        <span className="text-[9.5px] text-muted-foreground/70 uppercase">UID: {order.userId.slice(0, 8)}</span>
                      </td>
